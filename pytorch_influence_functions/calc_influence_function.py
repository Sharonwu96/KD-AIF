...
def calc_influence_single(model, train_loader, test_loader, test_id_num, gpu,
    ...
    #故意改错标签
    random.seed(42)
    #mislabel_data = random.sample(range(60000), 6000)
    #故意改错标签
    ...
    helpful = harmful[::-1]
    return list(influences), list(harmful), list(helpful), test_id_num
...
def calc_img_wise(config, model, train_loader, test_loader):
    ...
    for j in range(test_dataset_iter_len):       
        ...
        influences[str(j)]['num_in_dataset'] = i
        influences[str(j)]['time_calc_influence_s'] = end_time - start_time

        influences[str(j)]['influence'] = np.array(influence).tolist()
        influences[str(j)]['harmful'] = np.array(harmful).tolist()[:500]
        influences[str(j)]['helpful'] = np.array(helpful).tolist()[:500]
